...
module_hash() {
    ...
    # Читаем через stdin — иначе sha256sum включает в вывод путь, и хэш
    # начинает зависеть от того, где лежит репозиторий, что ломает любой
    # внешний пересчёт хэшей (см. документацию по ручному выставлению чекпоинтов).
    # Берём первые 16 символов агрегированного хэша.
    {
        sha256sum < "$LIB_DIR/$1"
